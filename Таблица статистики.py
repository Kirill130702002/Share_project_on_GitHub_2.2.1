...
class Vacancy:
    currency_to_rub = {
        "EUR": 59.90, "AZN": 35.68, "GEL": 21.74, "KGS": 0.76, "BYR": 23.91,
        "RUR": 1, "USD": 60.66, "UZS": 0.0055, "UAH": 1.64, "KZT": 0.13
    }
...
class DataSet:
    ...
    @staticmethod
    def print_statistic(salary_statistics_year, dynamics_count_vacancies, dynamics_salary_level_profession,
                        dynamics_salary_count_profession, salary_level_city, vacancy_rate_city):
        print('Динамика уровня зарплат по годам: {0}'.format(salary_statistics_year))
        print('Динамика количества вакансий по годам: {0}'.format(dynamics_count_vacancies))
        print('Динамика уровня зарплат по годам для выбранной профессии: {0}'.format(dynamics_salary_level_profession))
        print('Динамика количества вакансий по годам для выбранной профессии: {0}'
              .format(dynamics_salary_count_profession))
        # в порядке убывания:
        print('Уровень зарплат по городам (в порядке убывания): {0}'.format(salary_level_city))
        # В порядке убывания:
        print('Доля вакансий по городам (в порядке убывания): {0}'.format(vacancy_rate_city))
...
class Report:
    ...
    def generate_excel(self):
        ws1 = self.workbook.active #Присваеваем переменной ws1 текущий активный лист Workbook()
        ...
        for row in data:
            for row_index, cell in enumerate(row):
                if len(column_widths) > row_index:
                    if len(cell) > column_widths[row_index]:
                        column_widths[row_index] = len(cell)
                else:
        ...
        ws2 = self.workbook.create_sheet('Статистика по городам') #создается лист
